...
{
	/// <summary>
	/// Describes a 2-component position in cartesian space.
	/// </summary>
	...
	public struct Vec2 : IEquatable<Vec2>
	{
		...
		public readonly override bool Equals(object? obj) => (obj is Vec2 vec) && (vec == this);
		...
		public static void Project(in Vec2 l, in Vec2 r, out Vec2 o)
		{
			...
			float rx = r.X / rlen, ry = r.Y / rlen;
			float a = l.X * rx + l.Y * ry;
			o.X = rx * a;
			o.Y = ry * a;
		}
		...
		/// <param name="l">The first vector.</param>
		/// <param name="r">The second vector.</param>
		public static float AngleBetween(in Vec2 l, in Vec2 r) => MathF.Acos(Dot(l, r) / (l.Length * r.Length));
		...
		[MethodImpl(MethodImplOptions.AggressiveInlining)]
		public static bool NearlyEqual(in Vec2 l, in Vec2 r, float eps = MathHelper.FLOAT_EPSILON) =>
		...
		[MethodImpl(MethodImplOptions.AggressiveInlining)]
		public static implicit operator Vec2 (in (float x, float y) tup) => new Vec2(tup.x, tup.y);
		#endregion // Tuples
	}
}
