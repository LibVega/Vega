...
{
	/// <summary>
	/// Describes a 3-component position in cartesian space.
	/// </summary>
	...
	public struct Vec3 : IEquatable<Vec3>
	{
		...
		public readonly override bool Equals(object? obj) => (obj is Vec3 vec) && (vec == this);
		...
		public static void Project(in Vec3 l, in Vec3 r, out Vec3 o)
		{
			...
			float rx = r.X / rlen, ry = r.Y / rlen, rz = r.Z / rlen;
			float a = l.X * rx+ l.Y * ry + l.Z * rz;
			o.X = rx * a;
			o.Y = ry * a;
			o.Z = rz * a;
		}
		...
		/// <param name="l">The first vector.</param>
		/// <param name="r">The second vector.</param>
		public static float AngleBetween(in Vec3 l, in Vec3 r) => MathF.Acos(Dot(l, r) / (l.Length * r.Length));
		...
		[MethodImpl(MethodImplOptions.AggressiveInlining)]
		public static bool NearlyEqual(in Vec3 l, in Vec3 r, float eps = MathHelper.FLOAT_EPSILON) =>
			MathHelper.NearlyEqualFast(l.X, r.X, eps) && MathHelper.NearlyEqualFast(l.Y, r.Y, eps) &&
			MathHelper.NearlyEqualFast(l.Z, r.Z, eps);
		...
		[MethodImpl(MethodImplOptions.AggressiveInlining)]
		public static explicit operator Vec3 (in Point3 p) => new Vec3(p.X, p.Y, p.Z);
		[MethodImpl(MethodImplOptions.AggressiveInlining)]
