...
{
	/// <summary>
	/// Enumeration of the keys on a standard keyboard.
	/// </summary>
	public enum Keys : byte
	{
		...
		RightSuper = 93,
		/// <summary>The menu key (appears on some keyboards near the space bar, looks like a hamburger menu).</summary>
		Menu = 94,
	...
	/// </summary>
	public struct ModKeyMask : IEquatable<ModKeyMask>
	{
		#region Fields
		/// <summary>
		...
		/// <summary>
		/// If the bit for the left shift key is set.
		/// </summary>
		public bool LeftShift
		{
			readonly get => (Mask & 0x01) > 0;
			set => Mask = (byte)(value ? (Mask | 0x01) : (Mask & ~0x01));
		}
		/// <summary>
		/// If the bit for the left shift key is set.
		/// </summary>
		...
		{
			readonly get => (Mask & 0x02) > 0;
			set => Mask = (byte)(value ? (Mask | 0x02) : (Mask & ~0x02));
		}
		/// <summary>
		/// If the bit for the left shift key is set.
		/// </summary>
		public bool LeftAlt
		{
			readonly get => (Mask & 0x04) > 0;
			set => Mask = (byte)(value ? (Mask | 0x04) : (Mask & ~0x04));
		}
		/// <summary>
		/// If the bit for the left shift key is set.
		/// </summary>
		...
		{
			readonly get => (Mask & 0x10) > 0;
			set => Mask = (byte)(value ? (Mask | 0x10) : (Mask & ~0x10));
		}
		/// <summary>
		/// If the bit for the left shift key is set.
		/// </summary>
		...
		{
			readonly get => (Mask & 0x20) > 0;
			set => Mask = (byte)(value ? (Mask | 0x20) : (Mask & ~0x20));
		}
		/// <summary>
		/// If the bit for the left shift key is set.
		/// </summary>
		...
		{
			readonly get => (Mask & 0x40) > 0;
			set => Mask = (byte)(value ? (Mask | 0x40) : (Mask & ~0x40));
		}
		...
		public bool Shift
		{
			readonly get => (Mask & 0x11) > 0;
			set => Mask = (byte)(value ? (Mask | 0x11) : (Mask & ~0x11));
		}
		/// <summary>
		...
		public bool Control
		{
			readonly get => (Mask & 0x22) > 0;
			set => Mask = (byte)(value ? (Mask | 0x22) : (Mask & ~0x22));
		}
		/// <summary>
		...
		public bool Alt
		{
			readonly get => (Mask & 0x44) > 0;
			set => Mask = (byte)(value ? (Mask | 0x44) : (Mask & ~0x44));
		}
		#endregion // Fields
		...
		/// </summary>
		/// <param name="ls">Left shift bit.</param>
		/// <param name="lc">Left control bit.</param>
		/// <param name="la">Left alt bit.</param>
		/// <param name="rs">Right shift bit.</param>
		/// <param name="rc">Right control bit.</param>
		/// <param name="ra">Right alt bit.</param>
		public ModKeyMask(bool ls, bool lc, bool la, bool rs, bool rc, bool ra)
		{
			Mask = (byte)((ls ? 0x01 : 0x00) | (lc ? 0x02 : 0x00) | (la ? 0x04 : 0x00) |
						  (rs ? 0x10 : 0x00) | (rc ? 0x20 : 0x00) | (ra ? 0x40 : 0x00));
		}

		readonly bool IEquatable<ModKeyMask>.Equals(ModKeyMask other) => other.Mask == Mask;

		public readonly override bool Equals(object? obj) => (obj is ModKeyMask m) && m.Mask == Mask;
	...
	public static class KeysUtils
	{
		...
		internal static Keys Translate(int glfwKey)
		{
			return (glfwKey == -1) ? Keys.Unknown :
				   (glfwKey <= Glfw.KEY_GRAVE_ACCENT) ? _Keys[0][glfwKey - Glfw.KEY_SPACE] :
				   (glfwKey <= Glfw.KEY_WORLD_2) ? Keys.Unknown :
				   (glfwKey <= Glfw.KEY_END) ? _Keys[2][glfwKey - Glfw.KEY_ESCAPE] :
				   (glfwKey <= Glfw.KEY_MENU) ? _Keys[3][glfwKey - Glfw.KEY_CAPS_LOCK] : Keys.Unknown;
		}
